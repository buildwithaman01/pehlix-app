...
export const billingApi = {
  ...
  deletePayment: (id) =>
    apiClient.delete(`/payments/${id}`).then((r) => r.data.data),

  recordPayment: (invoiceId, data) =>
    apiClient.post(`/invoices/${invoiceId}/record-payment`, data).then((r) => r.data.data),
  ...
  updateInvoice: (invoiceId, data) =>
    apiClient.put(`/invoices/${invoiceId}`, data).then((r) => r.data.data),
};

export const doctorsApi = {
  getList: (params = {}) =>
    apiClient.get('/doctors', { params }).then((r) => r.data.data),
  ...
  delete: (id) =>
    apiClient.delete(`/doctors/${id}`).then((r) => r.data.data),
  ...
  payCommission: (id, data) =>
    apiClient.post(`/doctors/${id}/commission/pay`, data).then((r) => r.data.data),

  sendStatement: (id) =>
    apiClient.post(`/doctors/${id}/commission/statement`).then((r) => r.data.data),
...
export const adminApi = {
  getPlatformMetrics: () =>
    apiClient.get('/admin/platform/metrics').then((r) => r.data.data),
  ...
  suspendLab: (id, reason) =>
    apiClient.put(`/admin/labs/${id}/suspend`, { reason }).then((r) => r.data.data),

  restoreLab: (id) =>
    apiClient.put(`/admin/labs/${id}/restore`).then((r) => r.data.data),

  impersonate: (labId, userId, reason) =>
    apiClient.post(`/admin/labs/${labId}/impersonate`, { userId, reason }).then((r) => r.data.data),
  ...
  createLab: (data) =>
    apiClient.post('/admin/labs', data).then((r) => r.data.data),

  getFeatureFlags: () =>
    apiClient.get('/admin/feature-flags').then((r) => r.data.data),

  updateFeatureFlag: (name, data) =>
    apiClient.put(`/admin/feature-flags/${name}`, data).then((r) => r.data.data),

  sendAnnouncement: (data) =>
    apiClient.post('/admin/announcements', data).then((r) => r.data.data),

  getPlatformRevenue: (period) =>
    apiClient.get('/admin/platform/revenue', { params: { period } }).then((r) => r.data.data),

  getDeadLetterQueue: () =>
    apiClient.get('/admin/dead-letter-queue').then((r) => r.data.data),

  retryDeadLetterJob: (reportId) =>
    apiClient.post(`/admin/dead-letter-queue/${reportId}/retry`).then((r) => r.data.data),

  unlockOtpLockout: (phone) =>
    apiClient.post('/admin/otp/unlock', { phone }).then((r) => r.data.data),
};
