    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      if (originalRequest.url.includes('/auth/refresh')) {
        ...
        if (typeof window !== 'undefined') {
          ...
          // Add public paths that should NEVER redirect to login when session expires
          const isAuthPage = ['/login', '/register', '/otp', '/set-password'].includes(path);
          const isPublicPage = path === '/' || path.startsWith('/r/') || ['/about', '/pricing', '/contact', '/refund', '/terms', '/privacy'].some(p => path.startsWith(p));
