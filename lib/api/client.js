    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      if (originalRequest.url.includes('/auth/refresh')) {
        ...
        if (typeof window !== 'undefined') {
          ...
          const protectedPrefixes = [
            '/dashboard', '/patients', '/visits', '/results', '/billing', 
            '/home-collections', '/samples', '/staff', '/doctors', '/tests', 
            '/settings', '/audit-log', '/admin', '/portal', '/analytics', 
            '/bookings', '/critical', '/inventory', '/reports', '/whatsapp-outbox'
          ];
          const isProtectedPage = protectedPrefixes.some(p => path === p || path.startsWith(p + '/'));
      ...
      if (isRefreshing) {
        return new Promise((resolve, reject) => {
          addRefreshSubscriber((refreshError, token) => {
            if (refreshError) {
